export default {
    repository: {
        title: '',
        action: {
            create: 'Create Repository',
            confirm: 'Create',
            cancel: 'Cancel',
            delete: 'Delete',
            sync: 'Sync',
            configure: 'Configure',
            save: 'Save',
            restore: 'Restore',
            rename: 'Rename',
        },
        label: {
            name: 'Repository Name',
            nameHint: 'Leave empty to auto-derive from Remote URL',
            remoteUrl: 'Remote URL',
            requiresAuth: 'Requires Authentication',
            localType: 'Local Repository',
            remoteType: 'Remote Repository',
            synced: 'Synced',
            unsynced: 'Unsynced',
            unconfigured: 'Unconfigured',
            configuring: 'Configuring...',
            syncing: 'Syncing...',
            configureRemote: 'Configure Remote',
            sync: 'Sync',
            history: 'History',
            loading: 'Loading...',
            delete: 'Delete Repository',
            remoteUrlPlaceholder: 'Enter remote URL',
            userName: 'Username',
            userNamePlaceholder: 'Enter username',
            password: 'Password',
            passwordPlaceholder: 'Enter password',
            save: 'Save',
            rename: 'Rename',
            renamePlaceholder: 'Enter new repository name',
            renameRemoteUrl: 'Rename Repository',
        },
        message: {
            deleteConfirm: 'Confirm to delete repository {name}?',
            configureSuccess: 'Configuration saved',
            configureFailed: 'Failed to save configuration: {error}',
            syncSuccess: 'Sync completed successfully',
            syncFailed: 'Sync failed: {error}',
            syncNotConfigured: 'Sync not configured: {error}',
            syncChecking: 'Checking repository status...',
            syncCommitting: 'Committing local changes...',
            syncFetching: 'Fetching remote branches...',
            syncMerging: 'Merging latest changes...',
            syncPushing: 'Pushing branches...',
            noHistory: 'No commits found',
            restoreSuccess: 'Restored successfully',
            restoreFailed: 'Restore failed: {error}',
            restoreConfirm: 'Restore to this commit? A new restore commit will be created with the content from the selected state.',
            createFailed: 'Failed to create repository: {error}',
            cloneFailed: 'Failed to clone repository: {error}',
            deleteFailed: 'Failed to delete repository: {error}',
            historyLoadFailed: 'Failed to load history: {error}',
            renameSuccess: 'Repository renamed successfully',
            renameFailed: 'Rename failed: {error}',
        },
    },
    language: {
        title: '',
        action: {},
        label: {
            en: 'English',
            zh_CN: 'Simplified Chinese',
        },
        message: {},
    },
    theme: {
        title: '',
        action: {},
        label: {
            dark: 'Dark Theme',
            light: 'Light Theme',
        },
        message: {},
    },
    settings: {
        title: 'Settings',
        action: {},
        label: {
            appearance: 'Appearance',
            theme: 'Theme',
            fontSize: 'Font Size',
            language: 'Language',
            editor: 'Editor',
            autoSaveInterval: 'Auto-save Interval (seconds)',
            ai: 'AI Assistant',
            aiProvider: 'Provider',
            aiModel: 'Model',
            aiApiKey: 'API Key',
            aiEndpoint: 'Endpoint',
            aiProviderNone: 'None',
            aiModelPlaceholder: 'e.g. llama3, mistral',
            aiApiKeyPlaceholder: 'Enter API key',
        },
        message: {},
    },
    fileTree: {
        title: '',
        action: {
            newFile: 'New File',
            newFolder: 'New Folder',
            rename: 'Rename',
            delete: 'Delete',
            refresh: 'Refresh',
            cancel: 'Cancel',
            confirm: 'OK',
        },
        label: {
            rename: 'Rename',
            renamePlaceholder: 'Enter new name',
            deleteConfirmTitle: 'Confirm Delete',
            newFile: 'New File',
            fileNamePlaceholder: 'Enter file name',
            newFolder: 'New Folder',
            folderNamePlaceholder: 'Enter folder name',
        },
        message: {
            nameEmpty: 'Name cannot be empty',
            renameSuccess: 'Renamed successfully',
            renameFailed: 'Rename failed: {error}',
            deleteSuccess: 'Deleted successfully',
            deleteFailed: 'Delete failed: {error}',
            deleteConfirm: 'Are you sure you want to delete "{name}"? This action cannot be undone.',
            refreshSuccess: 'Refreshed successfully',
            refreshFailed: 'Refresh failed: {error}',
            fileNameEmpty: 'File name cannot be empty',
            folderNameEmpty: 'Folder name cannot be empty',
            fileCreateSuccess: 'File created successfully',
            folderCreateSuccess: 'Folder created successfully',
            createFailed: 'Create failed: {error}',
            loading: 'Loading...',
            loadFailed: 'Load failed: {error}',
            empty: 'Folder is empty',
        },
    },
    editor: {
        title: '',
        action: {
            save: 'Save',
            cancel: 'Cancel',
            confirm: 'Confirm',
        },
        label: {
            mode: {
                wysiwyg: 'WYSIWYG',
                split: 'Split',
                edit: 'Edit',
                preview: 'Preview',
            },
            format: {
                bold: 'Bold',
                italic: 'Italic',
                strikethrough: 'Strikethrough',
                code: 'Inline Code',
                h1: 'Heading 1',
                h2: 'Heading 2',
                h3: 'Heading 3',
                bulletList: 'Bullet List',
                orderedList: 'Ordered List',
                blockquote: 'Blockquote',
                link: 'Link',
                image: 'Image',
                hr: 'Horizontal Rule',
            },
            insertLink: 'Insert Link',
            linkText: 'Link Text',
            linkUrl: 'URL',
        },
        message: {
            emptyHint: 'Open a file from the file tree to start editing',
            saved: 'Saved',
            unsaved: 'Unsaved changes',
            saveFailed: 'Save failed: {error}',
            loadFailed: 'Failed to load file: {error}',
        },
    },
    ai: {
        title: '',
        action: {},
        label: {
            dialogTitle: 'AI Commit Message',
            filesChanged: 'Files changed',
            commitMessage: 'Commit Message',
            commitMessagePlaceholder: 'Edit commit message...',
            submit: 'Submit',
            cancel: 'Cancel',
            regenerate: 'Regenerate',
            provider: 'Provider',
        },
        message: {
            loading: 'Generating commit message...',
            invalidKey: 'Invalid API Key. Check settings.',
            timeout: 'Request timed out. Check your network.',
            serviceError: 'AI service error.',
            ollamaNotRunning: 'Cannot connect to Ollama. Start the service.',
        },
    },
}
